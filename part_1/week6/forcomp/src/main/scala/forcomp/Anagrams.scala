...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ((w.toLowerCase groupBy identity) map { case (chr, chrList) => (chr, chrList.length) } toList).sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {
      case ((chr, count), acc) => {
        acc ++ ( for { combination <- acc; n <- 1 to count }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y foldLeft SortedMap[Char, Int]() ++ x) {
      case (map, (chr, count)) => {
        val newCount = map(chr) - count
        if (newCount > 0) map updated (chr, newCount)
        else map - chr
    } }.toList
